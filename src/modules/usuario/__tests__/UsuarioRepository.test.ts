...
jest.mock('../UsuarioModel.js');
jest.mock('../../notificacao/NotificacaoModel.js');
...
describe('UsuarioRepository', () => {
  ...
  describe('listar', () => {
    ...
    it('deve listar usuários com filtros', async () => {
      ...
      UsuarioModel.paginate = jest
        .fn()
        .mockResolvedValue({ docs: [makeFakeUser()], total: 1 });
      const result = await repo.listar(req);
    ...
    it('deve lançar erro 500 se filterBuilder.build não for função', async () => {
      ...
      jest.dontMock('../UsuarioFilterBuilder.js');
    });
  });
  ...
  describe('deletar', () => {
    it('deve deletar usuário sem vínculo', async () => {
      ...
      UsuarioModel.findByIdAndDelete = jest
        .fn()
        .mockResolvedValue(makeFakeUser());
