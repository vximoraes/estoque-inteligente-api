...
describe('NotificacaoRepository', () => {
  ...
  it('deve buscar notificações por usuário', async () => {
    NotificacaoModel.paginate = jest
      .fn()
      .mockResolvedValue({ docs: [{ usuario: usuarioId }] });
  ...
  it('deve aplicar filtros de busca', async () => {
    NotificacaoModel.paginate = jest
      .fn()
      .mockResolvedValue({ docs: [{ visualizada: false }] });
  ...
  it('deve lançar erro ao cadastrar notificação com usuário inexistente', async () => {
    ...
    await expect(
      repository.criar({ mensagem: 'Teste', usuario: usuarioId }),
    ).rejects.toThrow('Usuário informado não existe.');
  });
  ...
  it('deve lançar erro ao buscar notificação inexistente', async () => {
    ...
    await expect(
      repository.buscarPorId('id_inexistente', 'user1'),
    ).rejects.toThrow('Notificação não encontrado(a).');
  });

  it('deve lançar erro ao atualizar notificação inexistente', async () => {
    NotificacaoModel.findByIdAndUpdate = jest
      .fn()
      .mockReturnValue({ populate: jest.fn().mockResolvedValue(null) });
    await expect(
      repository._atualizar('id_inexistente', { visualizada: true }),
    ).rejects.toThrow('Notificação não encontrado(a).');
  });
  ...
  it('deve lançar erro do banco em qualquer operação', async () => {
    NotificacaoModel.paginate = jest
      .fn()
      .mockRejectedValue(new Error('Falha banco'));
