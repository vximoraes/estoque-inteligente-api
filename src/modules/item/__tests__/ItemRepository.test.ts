...
jest.mock('../ItemModel.js');
jest.mock('../../movimentacao/MovimentacaoModel.js');
jest.mock('../ItemFilterBuilder.js');
...
describe('ItemRepository', () => {
  ...
  describe('listar', () => {
    it('deve retornar item por id', async () => {
      ...
      ItemModel.findOne = jest.fn().mockReturnValueOnce({
        populate: jest
          .fn()
          .mockResolvedValue({ toObject: () => ({ nome: 'C1', _id: 'id1' }) }),
      });
    ...
    it('deve listar itens com filtros', async () => {
      const req = {
        params: {},
        query: { nome: 'C1', page: 1, limite: 10 },
        user_id: 'user1',
      };
  ...
  describe('atualizar', () => {
    ...
    it('deve lançar erro 404 se item não encontrado', async () => {
      ...
      await expect(
        repository.atualizar('id1', { nome: 'Novo' }, req),
  ...
  describe('buscarPorId', () => {
    ...
    it('deve lançar erro 404 se não encontrar item', async () => {
      ...
      await expect(repository.buscarPorId('id1', false, req)).rejects.toThrow(
        CustomError,
      );
    });
  });
