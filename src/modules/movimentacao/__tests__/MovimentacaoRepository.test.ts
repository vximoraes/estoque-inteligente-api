jest.mock('../MovimentacaoModel.js');
...
describe('MovimentacaoRepository', () => {
  ...
  describe('criar', () => {
    it('deve criar movimentação e retornar populada', async () => {
      ...
      const mockPopulateFornecedor = jest
        .fn()
        .mockResolvedValue(populatedResult);
      const mockPopulateItem = jest
        .fn()
        .mockReturnValue({ populate: mockPopulateFornecedor });
      MovimentacaoModel.findById = jest
        .fn()
        .mockReturnValue({ populate: mockPopulateItem });
  ...
  describe('listar', () => {
    it('deve retornar movimentação por id', async () => {
      ...
      const mockPopulateItem = jest
        .fn()
        .mockReturnValue({ populate: mockPopulateFornecedor });
      MovimentacaoModel.findOne = jest
        .fn()
        .mockReturnValue({ populate: mockPopulateItem });
    ...
    it('deve aplicar filtros de busca e retornar resultado paginado', async () => {
      ...
      const repo = new MovimentacaoRepositoryWithMocks({
        movimentacaoModel: MovimentacaoModelMocked,
      });
      const req = {
  ...
  describe('buscarPorId', () => {
    ...
    it('deve lançar erro se o banco lançar exceção em criar', async () => {
      ...
      MovimentacaoModel.mockImplementation(() => {
        throw dbError;
      });
    ...
    it('deve lançar erro se o banco lançar exceção em listar', async () => {
      const dbError = new Error('DB error');
      MovimentacaoModel.findOne = jest.fn(() => {
        throw dbError;
      });
      const req = { params: { id: '1' }, user_id: 'user1' };
    ...
    it('deve lançar erro se o banco lançar exceção em buscarPorId', async () => {
      ...
      await expect(repository.buscarPorId('1', false, req)).rejects.toThrow(
        'DB error',
      );
    });
  });
