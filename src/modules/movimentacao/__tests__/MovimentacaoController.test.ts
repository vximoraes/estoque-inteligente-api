...
jest.mock('../MovimentacaoQuerySchema.js');
jest.mock('../MovimentacaoSchema.js');

jest.mock('../MovimentacaoService.js');
...
describe('MovimentacaoController', () => {
  ...
  describe('método listar', () => {
    ...
    it('deve validar os query params quando fornecidos', async () => {
      ...
      expect(MovimentacaoQuerySchema.parseAsync).toHaveBeenCalledWith(
        queryParams,
      );
      expect(mockService.listar).toHaveBeenCalledWith(mockReq);
      expect(CommonResponse.success).toHaveBeenCalledWith(
    ...
    it('deve lançar erro se a validação de ID falhar', async () => {
      ...
      await expect(controller.listar(mockReq, mockRes)).rejects.toThrow(
        validationError,
      );
    });
    it('deve lançar erro se a validação de query params falhar', async () => {
      ...
      const validationError = new Error('Query inválida');
