...
jest.mock('../GrupoRepository.js');
jest.mock('../../usuario/UsuarioRepository.js');
...
describe('GrupoService', () => {
  ...
  describe('criar', () => {
    ...
    it('deve propagar erro do repository', async () => {
      ...
      repositoryMock.buscarPorNome.mockRejectedValue(
        new Error('Database error'),
      );

      await expect(service.criar(grupoData)).rejects.toThrow('Database error');
    });
  });
  ...
  describe('validarPermissoes', () => {
    ...
    it('não consulta o banco quando não há permissoes', async () => {
    ...
    it('criar() valida as permissoes antes de persistir', async () => {
      const grupoData = {
        nome: 'Operadores',
  ...
  describe('listar', () => {
    ...
    it('deve propagar erro do repository', async () => {
      ...
      await expect(service.listar(req)).rejects.toThrow('Database error');
    });
  });
  ...
  describe('atualizar', () => {
    it('deve atualizar grupo existente', async () => {
      ...
      expect(repositoryMock.buscarPorNome).toHaveBeenCalledWith(
        'Novo Nome',
        grupoId,
      );
    ...
    it('deve lançar erro se nome já existe em outro grupo', async () => {
      ...
      await expect(
        service.atualizar(dadosAtualizacao, grupoId, user),
  ...
  describe('deletar', () => {
    ...
    it('deve propagar erro do repository', async () => {
      ...
      await expect(service.deletar(grupoId, user)).rejects.toThrow(
        'Database error',
      );
    });
  });
  ...
  describe('verificarGrupo', () => {
    it('deve verificar grupo sem lançar erro quando grupo não está na lista', async () => {
      ...
      await expect(
        service.verificarGrupo(user, grupoId),
    ...
    it('deve lançar erro quando grupo está na lista do usuário', async () => {
      ...
      await expect(service.verificarGrupo(user, grupoId)).rejects.toThrow(
        CustomError,
      );
    });
  });
