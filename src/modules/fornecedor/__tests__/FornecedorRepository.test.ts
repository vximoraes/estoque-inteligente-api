...
jest.mock('../FornecedorModel.js');
jest.mock('../../movimentacao/MovimentacaoModel.js');
...
describe('FornecedorRepository', () => {
  ...
  describe('listar', () => {
    ...
    it('deve listar fornecedores paginados', async () => {
      const req = {
        params: {},
        query: { page: 1, limite: 10, nome: 'abc' },
        user_id: 'user1',
      };
      FornecedorModel.paginate.mockResolvedValue({
        docs: [{ toObject: () => ({ nome: 'Fornecedor Z' }) }],
      });
      const result = await repository.listar(req);
      ...
      try {
        ...
        if (error.constructor.name === 'CustomError') {
          expect(error.message).toContain(
            'Erro interno no servidor ao processar Fornecedor',
          );
        }
      }

      jest.dontMock('../FornecedorFilterBuilder.js');
    });
  });
  ...
  describe('atualizar', () => {
    ...
    it('deve lançar erro se fornecedor não encontrado', async () => {
      ...
      await expect(repository.atualizar('id', {}, req)).rejects.toThrow(
        CustomError,
      );
    });
  });
  ...
  describe('buscarPorNome', () => {
    ...
    it('deve buscar fornecedor por nome ignorando id', async () => {
      ...
      const result = await repository.buscarPorNome(
        'Fornecedor B',
        'idIgnorado',
        req,
      );
  ...
  describe('buscarPorId', () => {
    it('deve retornar fornecedor por id', async () => {
      const req = { user_id: 'user1' };
      FornecedorModel.findOne = jest
        .fn()
        .mockResolvedValue({ nome: 'Fornecedor C' });
    ...
    it('deve lançar erro se fornecedor não encontrado', async () => {
      const req = { user_id: 'user1' };
      FornecedorModel.findOne = jest.fn().mockResolvedValue(null);
      await expect(repository.buscarPorId('id', false, req)).rejects.toThrow(
        CustomError,
      );
    });
  });
