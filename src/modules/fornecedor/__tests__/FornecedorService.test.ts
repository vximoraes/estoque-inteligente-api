...
jest.mock('../FornecedorRepository.js');
...
describe('FornecedorService', () => {
  ...
  describe('criar', () => {
    it('deve cadastrar fornecedor com nome único', async () => {
      ...
      repositoryMock.criar.mockResolvedValue(
        makeFornecedor({ usuario: 'user123' }),
      );
    ...
    it('deve lançar erro se nome já existir', async () => {
      ...
      await expect(
        service.criar({ nome: 'Fornecedor X' }, req),
  ...
  describe('listar', () => {
    it('deve retornar todos os fornecedores', async () => {
      const fornecs = [
        makeFornecedor(),
        makeFornecedor({ _id: 'forn2', nome: 'Fornecedor Y' }),
      ];
  ...
  describe('atualizar', () => {
    ...
    it('deve lançar erro se fornecedor não existir', async () => {
      ...
      await expect(
        service.atualizar('fornX', { nome: 'Qualquer' }),
      ).rejects.toThrow(CustomError);
    });
    it('deve lançar erro se tentar atualizar para nome já existente', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(makeFornecedor());
      repositoryMock.buscarPorNome.mockResolvedValue(
        makeFornecedor({ _id: 'forn2', nome: 'Duplicado' }),
      );
      await expect(
        service.atualizar('forn1', { nome: 'Duplicado' }),
      ).rejects.toThrow(CustomError);
    });
    it('deve lançar erro inesperado do repository', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(makeFornecedor());
      ...
      await expect(
        service.atualizar('forn1', { nome: 'Novo' }),
  ...
  describe('Métodos auxiliares', () => {
    it('validateNome lança erro se nome já existir', async () => {
      repositoryMock.buscarPorNome.mockResolvedValue(makeFornecedor());
      await expect(service.validateNome('Fornecedor X')).rejects.toThrow(
        CustomError,
      );
    });
    ...
    it('ensureSupplierExists lança erro se não existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(null);
      await expect(service.ensureSupplierExists('fornX')).rejects.toThrow(
        CustomError,
      );
    });
    it('ensureSupplierExists retorna fornecedor se existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(makeFornecedor());
      await expect(
        service.ensureSupplierExists('forn1'),
      ).resolves.toHaveProperty('_id', 'forn1');
    });
  });
});
