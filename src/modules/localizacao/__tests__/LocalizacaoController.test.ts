...
jest.mock('../LocalizacaoService.js');
jest.mock('../LocalizacaoSchema.js');
jest.mock('../LocalizacaoQuerySchema.js');
...
describe('LocalizacaoController', () => {
  ...
  describe('criar', () => {
    it('deve criar localização com dados válidos', async () => {
      const localizacao = {
        nome: 'Prateleira A',
        toObject: () => ({ nome: 'Prateleira A', _id: '1' }),
      };
    ...
    it('deve retornar erro 400 para dados inválidos', async () => {
      LocalizacaoSchema.parse.mockImplementation(() => {
        throw { name: 'ZodError' };
      });
  ...
  describe('atualizar', () => {
    ...
    it('deve retornar erro 400 para dados inválidos', async () => {
      req.params = { id: '1' };
      LocalizacaoIdSchema.parse.mockReturnValue('1');
      LocalizacaoUpdateSchema.parse.mockImplementation(() => {
        throw { name: 'ZodError' };
      });
    ...
    it('deve retornar erro 409 para nome já existente', async () => {
      req.params = { id: '1' };
      LocalizacaoIdSchema.parse.mockReturnValue('1');
      LocalizacaoUpdateSchema.parse.mockReturnValue({
        nome: 'Prateleira Existente',
      });
  ...
  describe('inativar', () => {
    it('deve inativar localização existente', async () => {
      req.params = { id: '1' };
      LocalizacaoIdSchema.parse.mockReturnValue('1');
      serviceMock.inativar.mockResolvedValue({
        nome: 'Prateleira A',
        ativo: false,
      });
    ...
    it('deve validar id ao inativar', async () => {
      ...
      LocalizacaoIdSchema.parse.mockImplementation(() => {
        throw { name: 'ZodError' };
      });
      await expect(controller.inativar(req, res)).rejects.toEqual(
        expect.objectContaining({ name: 'ZodError' }),
