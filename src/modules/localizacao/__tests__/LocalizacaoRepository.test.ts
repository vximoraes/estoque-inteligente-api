...
describe('LocalizacaoRepository', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    repository = new LocalizacaoRepository({
  ...
  describe('criar', () => {
    it('deve criar localização com nome único', async () => {
      const fakeLoc = {
        _id: 'loc1',
        nome: 'Estoque',
        save: jest.fn().mockResolvedValue({ _id: 'loc1', nome: 'Estoque' }),
      };
    ...
    it('deve lançar erro se o save falhar', async () => {
      LocalizacaoModel.mockImplementation(() => ({
        save: jest.fn().mockRejectedValue(new Error('DB error')),
      }));
      await expect(repository.criar({ nome: 'Estoque' })).rejects.toThrow(
        'DB error',
      );
    });
  });
  ...
  describe('listar', () => {
    it('deve retornar localização por id', async () => {
      const fakeLoc = {
        _id: 'loc1',
        nome: 'Estoque',
        toObject: () => ({ _id: 'loc1', nome: 'Estoque' }),
      };
    ...
    it('deve listar localizações com filtro de nome', async () => {
      ...
      const req = {
        params: {},
        query: { nome: 'Estoque', page: 1, limite: 10 },
      };
      const result = await repository.listar(req);
  ...
  describe('atualizar', () => {
    it('deve atualizar localização existente', async () => {
      ...
      LocalizacaoModel.findOneAndUpdate = jest.fn(() => ({
        lean: () => Promise.resolve({ _id: 'loc1', nome: 'Novo Nome' }),
      }));
      const result = await repository.atualizar(
        'loc1',
        { nome: 'Novo Nome' },
        req,
      );
    ...
    it('deve lançar erro 404 se localização não encontrada', async () => {
      ...
      await expect(
        repository.atualizar('notfound', { nome: 'X' }, req),
    ...
    it('deve lançar erro se o banco lançar exceção', async () => {
      ...
      await expect(
        repository.atualizar('loc1', { nome: 'X' }, req),
  ...
  describe('buscarPorNome', () => {
    it('deve buscar localização por nome', async () => {
      const req = { user_id: 'user1' };
      LocalizacaoModel.findOne = jest
        .fn()
        .mockResolvedValue({ _id: 'loc1', nome: 'Estoque' });
    ...
    it('deve buscar localização por nome ignorando id', async () => {
      ...
      LocalizacaoModel.findOne = jest
        .fn()
        .mockResolvedValue({ _id: 'loc2', nome: 'Estoque' });
    ...
    it('deve lançar erro se o banco lançar exceção', async () => {
      ...
      await expect(
        repository.buscarPorNome('Estoque', null, req),
      ).rejects.toThrow('DB error');
    });
  });
  ...
  describe('buscarPorId', () => {
    ...
    it('deve lançar erro 404 se localização não encontrada', async () => {
      ...
      LocalizacaoModel.findOne = jest.fn().mockResolvedValue(null);
      await expect(
        repository.buscarPorId('notfound', false, req),
    ...
    it('deve lançar erro se o banco lançar exceção', async () => {
      ...
      LocalizacaoModel.findOne = jest
        .fn()
        .mockRejectedValue(new Error('DB error'));
      await expect(repository.buscarPorId('loc1', false, req)).rejects.toThrow(
        'DB error',
      );
    });
  });
