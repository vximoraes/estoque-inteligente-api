...
jest.mock('../LocalizacaoRepository.js');
jest.mock('../../estoque/EstoqueModel.js');
...
describe('LocalizacaoService', () => {
  ...
  describe('criar', () => {
    it('deve cadastrar localização com nome único', async () => {
      ...
      repositoryMock.criar.mockResolvedValue(
        makeLocalizacao({ usuario: 'user123' }),
      );
    ...
    it('deve lançar erro se nome já existir', async () => {
      ...
      await expect(service.criar({ nome: 'Estoque' }, req)).rejects.toThrow(
        CustomError,
      );
    });
  });
  ...
  describe('listar', () => {
    it('deve retornar todas as localizações', async () => {
      const locs = [
        makeLocalizacao(),
        makeLocalizacao({ _id: 'loc2', nome: 'Sala' }),
      ];
  ...
  describe('atualizar', () => {
    it('deve atualizar nome de localização existente', async () => {
      ...
      repositoryMock.atualizar.mockResolvedValue(
        makeLocalizacao({ nome: 'Novo Nome' }),
      );
    ...
    it('deve lançar erro inesperado do repository', async () => {
      ...
      await expect(service.atualizar('loc1', { nome: 'Novo' })).rejects.toThrow(
        'DB error',
      );
    });
  });
  ...
  describe('inativar', () => {
    ...
    it('deve lançar erro se localização não existir', async () => {
      const locId = new mongoose.Types.ObjectId();
      repositoryMock.buscarPorId.mockResolvedValue(null);
      await expect(service.inativar(locId.toString(), {})).rejects.toThrow(
        CustomError,
      );
    });
    it('deve lançar erro se localização tiver itens ativos', async () => {
      ...
      repositoryMock.buscarPorId.mockResolvedValue(localizacao);
      EstoqueModel.find = jest.fn().mockReturnValue({
        populate: jest.fn().mockResolvedValue([{ item: { ativo: true } }]),
      });

      await expect(service.inativar(locId.toString(), {})).rejects.toThrow(
        'Localização possui estoque de itens ativos',
      );
    });
    it('deve lançar erro inesperado do repository', async () => {
      const locId = new mongoose.Types.ObjectId();
  ...
  describe('Métodos auxiliares', () => {
    it('validateNome lança erro se nome já existir', async () => {
      repositoryMock.buscarPorNome.mockResolvedValue(makeLocalizacao());
      await expect(service.validateNome('Estoque')).rejects.toThrow(
        CustomError,
      );
    });
    ...
    it('ensureLocationExists lança erro se não existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(null);
      await expect(service.ensureLocationExists('locX')).rejects.toThrow(
        CustomError,
      );
    });
    it('ensureLocationExists retorna localização se existir', async () => {
      const localizacao = makeLocalizacao();
      repositoryMock.buscarPorId.mockResolvedValue(localizacao);
      const result = await service.ensureLocationExists(
        localizacao._id.toString(),
      );
      expect(result).toHaveProperty('_id');
