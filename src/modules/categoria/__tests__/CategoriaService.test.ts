...
jest.mock('../CategoriaRepository.js');
...
describe('CategoriaService', () => {
  ...
  describe('criar', () => {
    ...
    it('deve lançar erro se nome já existir', async () => {
      ...
      await expect(service.criar({ nome: 'Passivo' })).rejects.toThrow(
        CustomError,
      );
    });
  });
  ...
  describe('listar', () => {
    it('deve retornar todas as categorias', async () => {
      const cats = [
        makeCategoria(),
        makeCategoria({ _id: 'cat2', nome: 'Ativo' }),
      ];
  ...
  describe('atualizar', () => {
    ...
    it('deve lançar erro inesperado do repository', async () => {
      ...
      await expect(service.atualizar('cat1', { nome: 'Novo' })).rejects.toThrow(
        'DB error',
      );
    });
  });
  ...
  describe('inativar', () => {
    it('deve inativar categoria existente sem itens ativos', async () => {
      const ItemModel = require('../../item/ItemModel.js').default;
      ItemModel.exists = jest.fn().mockResolvedValue(false);

      repositoryMock.buscarPorId.mockResolvedValue(makeCategoria());
      repositoryMock.atualizar.mockResolvedValue(
        makeCategoria({ ativo: false }),
      );
    ...
    it('deve lançar erro se categoria estiver vinculada a itens ativos', async () => {
      ...
      repositoryMock.buscarPorId.mockResolvedValue(makeCategoria());
      await expect(service.inativar('cat1', {})).rejects.toThrow(
        'Categoria vinculada a itens ativos',
      );
    });
    it('deve lançar erro inesperado do repository', async () => {
      const ItemModel = require('../../item/ItemModel.js').default;
  ...
  describe('Métodos auxiliares', () => {
    it('validateNome lança erro se nome já existir', async () => {
      repositoryMock.buscarPorNome.mockResolvedValue(makeCategoria());
      await expect(service.validateNome('Passivo')).rejects.toThrow(
        CustomError,
      );
    });
    ...
    it('ensureCategoryExists lança erro se não existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(null);
      await expect(service.ensureCategoryExists('catX')).rejects.toThrow(
        CustomError,
      );
    });
    it('ensureCategoryExists retorna categoria se existir', async () => {
      repositoryMock.buscarPorId.mockResolvedValue(makeCategoria());
      await expect(
        service.ensureCategoryExists('cat1'),
      ).resolves.toHaveProperty('_id', 'cat1');
    });
  });
});
