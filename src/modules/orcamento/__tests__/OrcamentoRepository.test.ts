...
describe('OrcamentoRepository', () => {
  ...
  it('deve listar todos os orçamentos com paginação', async () => {
    MockModel.paginate.mockResolvedValue({
      docs: [{ toObject: () => ({ _id: 'id' }) }],
    });
  ...
  it('deve lançar erro ao atualizar orçamento inexistente', async () => {
    ...
    const req = { user_id: 'user123' };
    await expect(
      repository.atualizar('id', { nome: 'Novo' }, req),
    ).rejects.toThrow();
  });
  ...
  it('deve lançar erro ao deletar orçamento inexistente', async () => {
    ...
    const req = { user_id: 'user123' };
    await expect(repository.deletar('id', req)).rejects.toThrow(
      'não encontrado',
    );
  });
  ...
  it('deve lançar erro ao adicionar item em orçamento inexistente', async () => {
    ...
    const req = { user_id: 'user123' };
    await expect(
      repository.adicionarItem('id', { nome: 'C' }, req),
  ...
  it('deve atualizar item', async () => {
    ...
    fake.save.mockResolvedValue(fake);
    const req = { user_id: 'user123' };
    const result = await repository.atualizarItem(
      'id',
      'cid',
      { nome: 'Novo' },
      req,
  ...
  it('deve lançar erro ao atualizar item inexistente', async () => {
    ...
    const req = { user_id: 'user123' };
    await expect(
      repository.atualizarItem('id', 'cid', { nome: 'Novo' }, req),
  ...
  it('deve lançar erro ao remover item de orçamento inexistente', async () => {
    MockModel.findOne.mockResolvedValue(null);
    const req = { user_id: 'user123' };
    await expect(repository.removerItem('id', 'cid', req)).rejects.toThrow(
      'não encontrado',
    );
  });
