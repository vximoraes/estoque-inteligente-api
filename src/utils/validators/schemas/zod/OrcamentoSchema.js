...
const ItemOrcamentoSchema = z.object({
  item: z.string().refine((val) => mongoose.Types.ObjectId.isValid(val), {
    message: 'ID do item deve ser um ObjectId válido',
  }),
  fornecedor: z.string().refine((val) => mongoose.Types.ObjectId.isValid(val), {
    ...
    .or(
      z
        ...
        .refine((val) => !isNaN(val) && val >= 0, {
          message:
            'Valor unitário deve ser um número válido maior ou igual a zero',
        }),
    ),
});
