import { ItemIdSchema, ItemQuerySchema } from '../../../../../../../utils/validators/schemas/zod/querys/ItemQuerySchema.js';
import mongoose from 'mongoose';

describe('ItemIdSchema', () => {
    it('deve validar um ObjectId correto', () => {
        const idValido = new mongoose.Types.ObjectId().toString();
        expect(() => ItemIdSchema.parse(idValido)).not.toThrow();
    });

    it('deve invalidar um ObjectId incorreto', () => {
        const idInvalido = '12345';
        expect(() => ItemIdSchema.parse(idInvalido)).toThrow('ID inválido');
    });
});
...
describe('ItemQuerySchema', () => {
    ...
    it('deve invalidar quantidade não numérica', () => {
        ...
        expect(() => ItemQuerySchema.parse(consulta)).toThrow('Quantidade deve ser um número inteiro');
    });
    ...
    it('deve invalidar estoque_minimo diferente de true/false', () => {
        ...
        expect(() => ItemQuerySchema.parse(consulta)).toThrow("Estoque mínimo deve ser 'true' ou 'false'");
    });
    ...
    it('deve invalidar ativo diferente de true/false', () => {
        ...
        expect(() => ItemQuerySchema.parse(consulta)).toThrow("Ativo deve ser 'true' ou 'false'");
    });
    ...
    it('deve validar paginação válida e usar defaults', () => {
        const resultado1 = ItemQuerySchema.parse({ page: '2', limite: '5' });
        expect(resultado1.page).toBe(2);
        expect(resultado1.limite).toBe(5);

        const resultado2 = ItemQuerySchema.parse({});
        expect(resultado2.page).toBe(1);
        expect(resultado2.limite).toBe(10);
    });

    it('deve invalidar paginação inválida', () => {
        expect(() => ItemQuerySchema.parse({ page: 'abc' })).toThrow();
        expect(() => ItemQuerySchema.parse({ page: '0' })).toThrow('Page deve ser um número inteiro maior que 0');
        expect(() => ItemQuerySchema.parse({ limite: '101' })).toThrow('Limite deve ser um número inteiro entre 1 e 100');
        expect(() => ItemQuerySchema.parse({ limite: '0' })).toThrow('Limite deve ser um número inteiro entre 1 e 100');
    });

    it('deve invalidar limite maior que 100', () => {
        const consulta = { ...consultaValida, limite: '101' };
        expect(() => ItemQuerySchema.parse(consulta)).toThrow('Limite deve ser um número inteiro entre 1 e 100');
    });
