...
describe('LocalizacaoQuerySchema', () => {
  ...
  it('retorna erro se nome for string vazia', async () => {
    await expect(
      LocalizacaoQuerySchema.parseAsync({ nome: '' }),
  ...
  it('retorna erro para paginação inválida', () => {
    expect(() => LocalizacaoQuerySchema.parse({ page: '0' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => LocalizacaoQuerySchema.parse({ page: '-1' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => LocalizacaoQuerySchema.parse({ limite: '0' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
    expect(() => LocalizacaoQuerySchema.parse({ limite: '101' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
    expect(() => LocalizacaoQuerySchema.parse({ page: 'abc' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => LocalizacaoQuerySchema.parse({ limite: 'abc' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
  });
  ...
  it('valida valores undefined e string vazia com defaults', () => {
    ...
    const parsed2 = LocalizacaoQuerySchema.parse({
      page: undefined,
      limite: undefined,
    });
