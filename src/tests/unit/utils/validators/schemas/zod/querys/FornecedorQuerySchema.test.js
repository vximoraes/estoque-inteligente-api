...
describe('FornecedorQuerySchema', () => {
  ...
  it('retorna erro se nome for string vazia', async () => {
    await expect(
      FornecedorQuerySchema.parseAsync({ nome: '' }),
  ...
  it('retorna erro para paginação inválida', () => {
    expect(() => FornecedorQuerySchema.parse({ page: '0' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => FornecedorQuerySchema.parse({ page: '-1' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => FornecedorQuerySchema.parse({ limite: '0' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
    expect(() => FornecedorQuerySchema.parse({ limite: '101' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
    expect(() => FornecedorQuerySchema.parse({ page: 'abc' })).toThrow(
      'Page deve ser um número inteiro maior que 0',
    );
    expect(() => FornecedorQuerySchema.parse({ limite: 'abc' })).toThrow(
      'Limite deve ser um número inteiro entre 1 e 100',
    );
  });

  it('valida valores undefined e string vazia com defaults', () => {
    const parsed1 = FornecedorQuerySchema.parse({
      page: undefined,
      limite: undefined,
    });
