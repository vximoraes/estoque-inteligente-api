...
describe('Messages Helper', () => {
  describe('Informative Messages', () => {
    ...
    test('deve retornar mensagem de usuário logado', () => {
      ...
      expect(messages.info.userLoggedIn(username)).toBe(
  ...
  describe('Error Messages', () => {
    ...
    test('deve retornar mensagem de operação cancelada pelo usuário', () => {
      expect(messages.error.operationCanceled).toBe(
    ...
    test('deve retornar mensagem de recurso não encontrado', () => {
      ...
      expect(messages.error.resourceNotFound(fieldName)).toBe(
        `${fieldName} não encontrado(a).`,
      );
    });

    test('deve retornar mensagem de página não disponível', () => {
      const page = 'Página';
      expect(messages.error.pageIsNotAvailable(page)).toBe(
    ...
    test('deve retornar mensagem de página sem dados', () => {
      const page = 'Página';
      expect(messages.error.pageNotContainsData(page)).toBe(
        `A página ${page} não contém dados.`,
      );
    });

    test('deve retornar mensagem de entrada duplicada', () => {
      const fieldName = 'Campo';
      expect(messages.error.duplicateEntry(fieldName)).toBe(
    ...
    test('deve retornar mensagem de recurso em uso', () => {
      const fieldName = 'Recurso';
      expect(messages.error.resourceInUse(fieldName)).toBe(
        `Recurso em uso em ${fieldName}.`,
      );
    });

    test('deve retornar mensagem de erro de servidor com recurso', () => {
      expect(messages.error.serverError('Usuário')).toBe(
        'Erro interno do servidor ao processar Usuário.',
      );
    });

    test('deve retornar mensagem de erro de validação com campo', () => {
      expect(messages.error.validationError('email')).toBe(
    ...
    test('deve retornar mensagem de erro de permissão', () => {
      ...
      expect(messages.error.permissionError(fieldName)).toBe(
    ...
    test('deve retornar mensagem de conflito de recurso', () => {
      ...
      expect(messages.error.resourceConflict(resource, conflictField)).toBe(
    ...
    test('deve retornar mensagem de erro de autenticação', () => {
      const fieldName = 'Usuário';
      expect(messages.error.authenticationError(fieldName)).toBe(
        `Erro de autenticação em ${fieldName}.`,
      );
    });

    test('deve retornar mensagem de acesso não autorizado com e sem recurso', () => {
      expect(messages.error.unauthorized('Usuário')).toBe(
        'Acesso não autorizado: Usuário.',
      );
      expect(messages.error.unauthorized()).toBe(
        'Acesso não autorizado. Faça login para continuar.',
      );
    });

    test('deve retornar mensagem de permissão negada com e sem ação', () => {
      expect(messages.error.forbidden('exclusão')).toBe(
        'Permissão negada para exclusão.',
      );
      expect(messages.error.forbidden()).toBe('Permissão negada.');
    });

    test('deve retornar mensagem de badRequest com e sem campo', () => {
      expect(messages.error.badRequest('campoTeste')).toBe(
        'Requisição inválida: campoTeste',
      );
      expect(messages.error.badRequest()).toBe(
        'Requisição inválida. Verifique os parâmetros fornecidos.',
      );
    });
  });
  ...
  describe('Validation Messages', () => {
    test('deve retornar mensagem de campo obrigatório', () => {
      const fieldName = 'Campo';
      expect(messages.validation.generic.fieldIsRequired(fieldName)).toBe(
    ...
    test('deve retornar mensagem de campo repetido', () => {
      const fieldName = 'Campo';
      expect(messages.validation.generic.fieldIsRepeated(fieldName)).toBe(
    ...
    test('deve retornar mensagem de formato de entrada inválido', () => {
      const fieldName = 'Campo';
      expect(messages.validation.generic.invalidInputFormat(fieldName)).toBe(
    ...
    test('deve retornar mensagem de valor inválido', () => {
      const fieldName = 'Campo';
      expect(messages.validation.generic.invalid(fieldName)).toBe(
    ...
    test('deve retornar mensagem de valor não encontrado', () => {
      const fieldName = 'Campo';
      expect(messages.validation.generic.notFound(fieldName)).toBe(
    ...
    test('deve retornar mensagem de valor deve ser um dos especificados', () => {
      ...
      expect(messages.validation.generic.mustBeOneOf(fieldName, values)).toBe(
    ...
    test('deve retornar mensagem de recurso criado com sucesso', () => {
      const fieldName = 'Recurso';
      expect(messages.validation.generic.resourceCreated(fieldName)).toBe(
    ...
    test('deve retornar mensagem de recurso atualizado com sucesso', () => {
      const fieldName = 'Recurso';
      expect(messages.validation.generic.resourceUpdated(fieldName)).toBe(
    ...
    test('deve retornar mensagem de recurso excluído com sucesso', () => {
      const fieldName = 'Recurso';
      expect(messages.validation.generic.resourceDeleted(fieldName)).toBe(
    ...
    test('deve retornar mensagem de recurso já existente', () => {
      const fieldName = 'Recurso';
      expect(messages.validation.generic.resourceAlreadyExists(fieldName)).toBe(
        `${fieldName} já existe.`,
      );
    });
    ...
    test('deve retornar mensagem de recurso com referência', () => {
      ...
      expect(
        messages.validation.reference.resourceWithReference(
          resource,
          reference,
        ),
      ).toBe(`${resource} com referência em ${reference}. Exclusão impedida.`);
    });

    test('deve retornar mensagem de CPF inválido', () => {
      expect(messages.validation.custom.invalidCPF.message).toBe(
        'CPF inválido. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de CNPJ inválido', () => {
      expect(messages.validation.custom.invalidCNPJ.message).toBe(
        'CNPJ inválido. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de CEP inválido', () => {
      expect(messages.validation.custom.invalidCEP.message).toBe(
        'CEP inválido. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de número de telefone inválido', () => {
      expect(messages.validation.custom.invalidPhoneNumber.message).toBe(
        'Número de telefone inválido. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de email inválido', () => {
      expect(messages.validation.custom.invalidMail.message).toBe(
        'Email no formato inválido.',
      );
    });

    test('deve retornar mensagem de ano inválido', () => {
      expect(messages.validation.custom.invalidYear.message).toBe(
        'Ano inválido. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de data inválida', () => {
      expect(messages.validation.custom.invalidDate.message).toBe(
        'Data inválida. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de quilometragem inicial inválida', () => {
      expect(messages.validation.custom.invalidKilometerInitial.message).toBe(
        'Quilometragem inicial inválida.',
      );
    });

    test('deve retornar mensagem de quilometragem inválida', () => {
      expect(messages.validation.custom.invalidKilometer.message).toBe(
        'Quilometragem inválida.',
      );
    });

    test('deve retornar mensagem de data de início inválida (passada)', () => {
      expect(messages.validation.custom.invalidDatePast.message).toBe(
        'Data do início deve ser uma data atual ou futura.',
      );
    });

    test('deve retornar mensagem de data de conclusão inválida (futura)', () => {
      expect(messages.validation.custom.invalidDateFuture.message).toBe(
        'A data de conclusão deve ser maior do que a data de início!',
      );
    });

    test('deve retornar mensagem de data de início inválida (atual ou passada)', () => {
      expect(messages.validation.custom.invalidDateCurrent.message).toBe(
        'Data do início deve ser uma data atual ou passada.',
      );
    });

    test('deve retornar mensagem de vigência com período maior que 12 meses', () => {
      expect(messages.validation.custom.invalidDateMonths.message).toBe(
    ...
    test('deve retornar mensagem de data de nascimento inválida', () => {
      expect(messages.validation.custom.invalidDataNascimento.message).toBe(
        'Data de nascimento deve ser uma data passada e maior que 18 anos.',
      );
    });

    test('deve retornar mensagem de data de admissão inválida', () => {
      expect(messages.validation.custom.invalidDataAdmissao.message).toBe(
        'Data de admissão deve ser uma data atual ou passada.',
      );
    });

    test('deve retornar mensagem de ano/semestre inválido', () => {
      expect(messages.validation.custom.invalidYearSemester.message).toBe(
        'Ano/semestre. Verifique o formato e tente novamente.',
      );
    });

    test('deve retornar mensagem de data de início do semestre inválida', () => {
      expect(messages.validation.custom.invalidYearStartSemester.message).toBe(
  ...
  describe('Authentication Messages', () => {
    test('deve retornar mensagem de falha na autenticação', () => {
      expect(messages.auth.authenticationFailed).toBe(
    ...
    test('deve retornar mensagem de usuário não encontrado', () => {
      const userId = '123';
      expect(messages.auth.userNotFound(userId)).toBe(
        `Usuário com ID ${userId} não encontrado.`,
      );
    });

    test('deve retornar mensagem de permissão insuficiente', () => {
      expect(messages.auth.invalidPermission).toBe(
    ...
    test('deve retornar mensagem de email já existente', () => {
      ...
      expect(messages.auth.emailAlreadyExists(email)).toBe(
        `O endereço de email ${email} já está em uso.`,
      );
    });

    test('deve retornar mensagem de credenciais inválidas', () => {
      expect(messages.auth.invalidCredentials).toBe(
